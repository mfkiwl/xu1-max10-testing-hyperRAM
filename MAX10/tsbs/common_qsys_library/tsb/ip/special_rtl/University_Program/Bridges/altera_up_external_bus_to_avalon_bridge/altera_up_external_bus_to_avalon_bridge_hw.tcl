...
set_parameter_property addr_size ALLOWED_RANGES {4096 2048 1024 512 256 128 64 32 16 8 4 2 1}
...
proc elaborate {} {
	...
	if { $addr_size_multiplier == "Mbytes" } {
					set addr_multiplier 1048576;
				} elseif { $addr_size_multiplier == "Kbytes" } {
					set addr_multiplier 1024
				} else {
					set addr_multiplier 1
				}

	set addr_width [ expr int (ceil ((log ($addr_size) + log($addr_multiplier)) / (log (2)))) ]
...
proc generate {} {
   ...
   set aw	[ format "AW:%.0f"	[ expr (ceil ((log ($addr_size) + log($addr_multiplier)) / (log (2)))) - 1 ] ]
