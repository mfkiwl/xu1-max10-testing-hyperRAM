...
# *******************************************************************************
#
# MODULE_NAME = four_lane_seriallite_constraints.tcl
# COMPANY     = Altera Corporation, Altera Ottawa Technology Center
# WEB         = www.altera.com
#
# FUNCTIONAL_DESCRIPTION :
#    SLITE-II Constraint file. Used by Quartus II.
#    To use this constraint file (assuming the name of this file is four_lane_seriallite_constraints.tcl ):
#       -create a Quartus II project
#       - copy this constraint file into the project directory
#       - open the project
#       - run this constraint file
#           in the TCL Console:  >source four_lane_seriallite_constraints.tcl
#           or,
#           select Tools / TCL Scripts / four_lane_seriallite_constraints.tcl
...
if { $using_sdc_comp == 0 } {
   ...
   set_global_assignment -name SDC_FILE four_lane_seriallite.sdc
}
...
set_instance_assignment -name CUT ON -from * -to "*four_lane_seriallite_slite2_top*meta*"
set_instance_assignment -name CUT ON -from * -to "*four_lane_seriallite_slite2_top*wr_*sync*"
set_instance_assignment -name CUT ON -from * -to "*four_lane_seriallite_slite2_top*rd_*sync*"
     #set_false_path -from [get_pins -hierarchical *link_up|clk*]
     set_instance_assignment -name CUT ON -from "*four_lane_seriallite_slite2_top*linksm_inst|link_up" -to *
     # Cut the "metastable" flops.
     set_instance_assignment -name CUT ON -from * -to "*meta*"    
#set_false_path -from [get_pins -hierarchical *reset_d[2]|clk*]
set_instance_assignment -name CUT ON -from "*four_lane_seriallite_slite2_top*reset_syncer|reset_d[2]" -to *
